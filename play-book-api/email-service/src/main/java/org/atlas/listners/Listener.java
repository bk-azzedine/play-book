...
public class Listener {
    ...
    private void handleTeamInvite(HashMap<String, Object> map) throws MessagingException {
        emailService.sendInviteEmail(
                ...
                map.get("team_description").toString(),
                map.get("team_role").toString()
        );
        logger.info("Sent team invite email to: {}", map.get("email"));
    }
    private void handleForgotPassword(HashMap<String, Object> map) throws MessagingException {
        emailService.sendForgotPasswordEmail(
                map.get("to").toString(),
                Template.FORGOT_PASSWORD,
                map.get("reset_code").toString()

        );
        logger.info("Sent reset password email to: {}", map.get("email"));
    }
}
