...
export class UserService {
  ...
  registerWithInvite(user: User, invite:string): Observable<{ token: string, user: User }> {
    return this.httpService.post<any>(`user/register/${invite}`, user).pipe(
      map((res: HttpResponse<any>) => {
        const token: string | null = res.headers.get('Authorization');
        const user: User  = res.body;
        if (!token) {
  ...
  checkEmail(email: string): Observable<boolean> {
    return this.httpService.get<boolean>(`user/check/${email}`, null).pipe(
      map((res) => {
        return res;
      })
    )
