...
export class AuthService {
  ...
  forgotPassword(userEmail: string): Observable<string> {
    ...
    return this.httpService.post<any>('auth/forgot-password', payload).pipe(
      map((result: any) => {
        if (!result ) {
          throw new Error('response is missing from the response');
        }
        return result;
      }),
      catchError(error => {
  ...
  validateReset(code: string): Observable<string> {
    const payload = { code: code };

    return this.httpService.post<any>('auth/forgot-password/validate', payload).pipe(
      map((result: any) => {
        if (!result ) {
          throw new Error('response is missing from the response');
        }
      ...
      catchError(error => {
        console.error('Validation code failed:', error);
        return throwError(() => error);
      })
    );
  }
  updatePassword(password: string, code: string): Observable<any> {
    const payload = { code: code, password: password };

    return this.httpService.post<any>('auth/forgot-password/reset-password', payload).pipe(
      map((result: any) => {
        if (!result) {
          throw new Error('Response is missing from the server');
        }
        return result;
      }),
      catchError(error => {
